...
Description:
  ...
  to detect torn frames.  In mapped memory, BuffT is preceeded by GTR2MappedBufferVersionBlock.

  GTR2MappedBufferVersionBlock variables allow users who need to ensure consistent buffer view to check if
...
public:
  ...
  bool Initialize(bool mapGlobally)
  {
    ...
    memset(mpMappedView, 0, sizeof(GTR2MappedBufferVersionBlock) + sizeof(BuffT));
  ...
  void BeginUpdate()
  {
    ...
    if (READ_BUFFER_SUPPORTED_LAYOUT_VERSION != 0L) {
      DEBUG_MSG(
        DebugLevel::Warnings, DebugSource::General, "BeginUpdate: skipping as it does not apply to Read buffer");
      return;
    }

    // Fix up out of sync situation.
    if (mpWriteBuffVersionBlock->mVersionUpdateBegin != mpWriteBuffVersionBlock->mVersionUpdateEnd) {
      if (Utils::IsFlagOn(CrewChiefPlugin::msDebugOutputLevel, DebugLevel::Synchronization)) {
        DEBUG_MSG(DebugLevel::Synchronization,
                  DebugSource::MappedBufferSource,
                  "BeginUpdate: versions out of sync.  Version Begin:%ld  End:%ld",
                  mpWriteBuffVersionBlock->mVersionUpdateBegin,
                  mpWriteBuffVersionBlock->mVersionUpdateEnd);
      }
  ...
  void EndUpdate()
  {
    ...
    if (mpWriteBuffVersionBlock->mVersionUpdateBegin != mpWriteBuffVersionBlock->mVersionUpdateEnd) {
      if (Utils::IsFlagOn(CrewChiefPlugin::msDebugOutputLevel, DebugLevel::Synchronization)) {
        DEBUG_MSG(DebugLevel::Synchronization,
                  DebugSource::MappedBufferSource,
                  "EndUpdate: versions out of sync.  Version Begin:%ld  End:%ld",
                  mpWriteBuffVersionBlock->mVersionUpdateBegin,
                  mpWriteBuffVersionBlock->mVersionUpdateEnd);
      }
  ...
  bool VerifyBusyOrUnchanged(unsigned long versionUpdateBegin, unsigned long versionUpdateEnd)
  {
    ...
    if (versionUpdateBegin != versionUpdateEnd) {
      if (Utils::IsFlagOn(CrewChiefPlugin::msDebugOutputLevel, DebugLevel::Synchronization)) {
        DEBUG_MSG(DebugLevel::Synchronization,
                  DebugSource::MappedBufferSource,
                  "VerifyBusyOrUnchanged: versions out of sync.  Version Begin:%ld  End:%ld",
                  versionUpdateBegin,
                  versionUpdateEnd);
      }
      return true;
  ...
  bool ReadUpdate()
  {
    ...
    GTR2MappedBufferVersionBlock versionBegin;
    memcpy(&versionBegin, mpWriteBuffVersionBlock, sizeof(GTR2MappedBufferVersionBlock));
    ...
    GTR2MappedBufferVersionBlock versionEnd;
    memcpy(&versionEnd, mpWriteBuffVersionBlock, sizeof(GTR2MappedBufferVersionBlock));
  ...
  void ReleaseResources()
  {
    ...
    if (mpMappedView != nullptr && !::UnmapViewOfFile(mpMappedView)) {
...
private:
  ...
  HANDLE MapMemoryFile(char const* const fileName,
                       bool dedicatedServerMapGlobally,
                       LPVOID& pMappedView,
                       GTR2MappedBufferVersionBlock*& pBufVersionBlock,
                       BuffT*& pBuf) const
  {
    ...
    if (!isDedicatedServer                                       // Regular client use.
      ...
      hMap = ::CreateFileMappingA(INVALID_HANDLE_VALUE,
                                  ...
                                  PAGE_READWRITE,
                                  0 /*dwMaximumSizeLow*/,
                                  sizeof(GTR2MappedBufferVersionBlock) + sizeof(BuffT),
                                  mappingName);
    } else {
      ...
      auto ret
        = ConvertStringSecurityDescriptorToSecurityDescriptor("D:P(A;OICI;GA;;;SY)(A;OICI;GA;;;BA)(A;OICI;GA;;;WD)",
                                                              SDDL_REVISION_1,
                                                              &security.lpSecurityDescriptor,
                                                              nullptr);

      auto onExit = Utils::MakeScopeGuard([&]() { ::LocalFree(security.lpSecurityDescriptor); });

      if (!ret) {
        DEBUG_MSG(DebugLevel::Errors,
                  DebugSource::General,
                  "Failed to create security descriptor for mapping: '%s'",
                  mappingName);
        CrewChiefPlugin::TraceLastWin32Error();
        return nullptr;
      }
    ...
    // Set pointers up.
    pBufVersionBlock = static_cast<GTR2MappedBufferVersionBlock*>(mpMappedView);
    pBuf = reinterpret_cast<BuffT*>(static_cast<char*>(pMappedView) + sizeof(GTR2MappedBufferVersionBlock));
    assert((reinterpret_cast<char*>(pBufVersionBlock) + sizeof(GTR2MappedBufferVersionBlock))
...
public:
  ...
  GTR2MappedBufferVersionBlock* mpWriteBuffVersionBlock = nullptr;
