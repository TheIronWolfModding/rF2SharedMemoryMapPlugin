...
struct TelemInfoBase
{
  ...
  TelemVect3
    mOriX; // top row of orientation matrix (also converts local vehicle vectors into world X using dot product)
  TelemVect3
    mOriY; // mid row of orientation matrix (also converts local vehicle vectors into world Y using dot product)
  TelemVect3
    mOriZ; // bot row of orientation matrix (also converts local vehicle vectors into world Z using dot product)
...
struct TelemInfoV2
  : public TelemInfoBase // for noobs: TelemInfoV2 contains everything in TelemInfoBase, plus the following:
{
  // state/damage info
...
struct GraphicsInfoV2
  : public GraphicsInfo // for noobs: GraphicsInfoV2 contains everything in GraphicsInfo, plus the following:
{
  float mAmbientRed;
  float mAmbientGreen;
  float mAmbientBlue;
...
struct VehicleScoringInfoV2 : public VehicleScoringInfo // for noobs: VehicleScoringInfoV2 contains everything in
                                                        // VehicleScoringInfo, plus the following:
{
...
struct ScoringInfoV2
  : public ScoringInfoBase // for noobs: ScoringInfoV2 contains everything in ScoringInfoBase, plus the following:
{
  // Game phases:
...
public:
  ...
  virtual bool ForceFeedback(float& forceValue)
  {
    return (false);
  } // alternate force feedback computation - return true if editing the value
};

class InternalsPluginV2
  : public InternalsPlugin // for noobs: InternalsPluginV2 contains everything in InternalsPlugin, plus the following:
{
public:
  // SCORING OUTPUT
...
class InternalsPluginV3 : public InternalsPluginV2 // for noobs: InternalsPluginV3 contains everything in
                                                   // InternalsPluginV2 (and InternalsPlugin), plus the following:
{
public:
  ...
  {
    return (false);
  } // to use our commentary event system, fill in data and return true
};
