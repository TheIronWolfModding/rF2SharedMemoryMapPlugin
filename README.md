...
This plugin mirrors the internal state exposed by rFactor 2 into shared memory buffers.  Essentially, this is direct memcpy of rFactor 2 internals (except for extended stuff implemented to workaround API limitations/bugs).  Plugin also allows some level of input back into the rFactor 2 API.
...
This plugin is carefully implemented with an intent of becoming a shared component for reading rF2 internals.  For read operations, it can handle any number of clients without slowing down the rF2 plugin thread.  A lot of work was done to ensure it is as efficient and reliable as possible.

rFactor 2 API has some limitations/bugs, and the plugin tries to deal with that by deriving some information: basic accumulated damage info, tracking session transitions and optionally reading game memory directly.
...
## Features
The plugin offers optional weak synchronization by using version variables on each of the output buffers.

The plugin is built using VS Community Edition, targeting VC12 (VS 2013) runtime, since rF2 comes with VC12 redist.
...
Note: by default `Graphics` and `Weather` are unsubscribed.

## Input Buffers
Note to cheaters who dare to contact me with questions: none of this can be used to control the vehicle, rFactor does not allow it.

The plugin supports sending input to the game (using rFactor 2 API).  Please use this stuff with extreme care - it can cause game freezes and unexpected behavior.

Monitor app includes sample code that uses Input buffers.

Note: designed with only one write client in mind.  Although multiple clients should be able to coexist fine, they will need to be mindful of buffer update clashes, the plugin does not moderate that part.

### HWControl input
Allows sending restricted set of inputs to rF2.  Mostly useful for pit menu interaction.  To repeat: it cannot be used to control the vehicle, even the headlights.
...
### Rules Control input
This is an experimental control buffer.  It allows sending Rules input to the game.  The idea was that it might make developing custom rules plugin easier, by being able to change logic in an external app/script without rebuilding plugin/restarting rF2.  

However, this is experimental, because I am not positive this approach is going to "fly"/is reliable.  The reason for that is that when done inside of a plugin, rules are applied synchronously: the game sends rules input, and allows updating that input in the same callback, meaning synchronously.  Using the shared memory plugin this process is not synchronous: the game sends rules in a function call, the plugin picks up the rules update from the input buffer in a separate function call, and will apply the update next time the game calls rule update function.  So there's a gap between the game output rules and the requested input rules.
...
It is possible to configure which buffers get updated and which don't.  This is done via `UnsubscribedBuffersMask` value in the `CustomPluginVariables.json` file.  To specify buffers to unsubscribe from, add the desired flag values up.
...
- Note: unsubscribing from the `Extended` buffer updates is not supported.
- Note: unsubscribing from `Scoring` will disable `Plugin Control` input.
...
* The plugin assumes that delta Elapsed Time in a telemetry update frame cannot exceed 2ms (which effectively limits telemetry refresh rate to 50FPS).

## Monitor
The plugin comes with the rF2SMMonitor program that shows how to access exposed internals from C# program.  It is also useful for visualization of shared memory contents and general understanding of rFactor 2 internals.
...
## Memory Buffer Uses
  ...
  * Advanced:  If you would like to make sure you're not reading a torn (partially overwritten) frame, see `rF2SMMonitor.MappedBuffer<>.GetMappedData` for sample implementation.

## Dedicated server use
If running in a dedicated server process, each shared memory buffer name has the server PID appended.  If DedicatedServerMapGlobally preference is set to 1, the plugin will attempt creating shared memory buffers in the Global section.  Note that "Create Global Objects" permission is needed on the user account running dedicated server.
...
Please also be aware, that Crew Chief will always ship with the latest version of the .dll and will overwrite the .dll to match its version.  I do not expect compatibility to break without the game changing its model, aside from `rF2Extended` buffer, which contains stuff not directly exposed by the game.  Every time they layout of the memory changes, either of the first two digits in the Plugin version is incremented, which means clients might need an update.  The Monitor app is kept in sync with plugin.
...
* Headlight controls: https://forum.studio-397.com/index.php?threads/headlight-controls.63904/
* Python client API: https://forum.studio-397.com/index.php?threads/rf2-shared-memory-tools-for-developers.54282/page-5#post-988673
